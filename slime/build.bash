...
printf 'Using %s=%s\n' \
  TORCH_CUDA_ARCH_LIST "${TORCH_CUDA_ARCH_LIST:-<None>}" \
...
NVCC_APPEND_FLAGS="${NVCC_APPEND_FLAGS} --threads 4" \
  ...
  --config-settings "--build-option=--cpp_ext --cuda_ext --parallel ${MAX_JOBS:-4}" \
  . \
  |& _LOG apex.log \
  | sed '/^Killed$/{p; Q1}'
)
...
cd sglang
git apply /build/v0.5.9/sglang.patch --3way
