...
cd /root/Megatron-LM
git apply /wheels/v0.5.9/megatron.patch --3way
...
# Fix SLIME string version comparison bug: "2.10" < "2.6" lexicographically.
# SLIME uses str(torch.__version__) >= "2.6" to pick between pg_options (old)
# and backend_options (new). This breaks for torch >= 2.10 because '1' < '6'.
# Replace with tuple-based comparison.
# Remove once upstream merges the fix.
# https://github.com/THUDM/slime/issues/1666
# https://github.com/THUDM/slime/pull/1667
grep -q 'str(torch.__version__) >= "2.6"' /root/slime/slime/utils/distributed_utils.py || \
  { echo "ERROR: SLIME version comparison pattern not found — upstream may have fixed this"; exit 1; }
sed -i 's/str(torch.__version__) >= "2.6"/(int(torch.__version__.split(".")[0]), int(torch.__version__.split(".")[1])) >= (2, 6)/' \
  /root/slime/slime/utils/distributed_utils.py
